...
def load_image_from_file():
    ...
    If it fails, returns (None, None) and flashes an error message.
    """
...
def load_image_from_capture():
    """
    Loads the Base64 encoded image data from the form field 'captured_image'.
    Returns (image, filename) or (None, None) and flashes an error message if it fails.
    """
    ...
    if captured:
        try:
            ...
            flash("Captured image format error!")
            return None, None
        return image, "captured.jpg"
    flash("Captured image data not found!")
    return None, None
...
def upload():
    """
    Route for processing file uploads.
    POST requests: Reads the uploaded file, processes the image, and stores the result.
    GET requests: Displays the most recently uploaded image (if available).
    """
    if request.method == "POST":
        image_obj, filename = load_image_from_file()
        ...
        return redirect(url_for("upload", uploaded=new_id))

    # GET request: if a recently uploaded image exists, display it.
...
def camera():
    """
    Route for processing camera captures.
    POST requests: Reads the captured image, processes it, and stores the result.
    GET requests: Displays the most recent captured image (if available).
    """
    if request.method == "POST":
        image_obj, filename = load_image_from_capture()
        if image_obj is None:
            return redirect(request.url)
        new_id = process_upload(image_obj, filename)
        if new_id is None:
        ...
        flash("Captured image uploaded and processed successfully!")
        return redirect(url_for("camera", uploaded=new_id))
    ...
    return render_template("camera.html", files=files)
